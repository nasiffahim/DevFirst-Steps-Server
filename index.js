  ...
  try {
    ...
    app.post("update-activity", async (req, res) => {
      try{
        ...
        const user = await users.findOne({_id: new ObjectId(email) });
        if(!user){
          return res.status(404).json({ message: "User not found" });
        }
       ...
       res.status(200).json({ message: "Activity updated", points: newPoints, badge: newBadge });

       res.json({ message: "Activity updated", points: newPoints, badge: newBadge });
      }
      catch(error){
    ...
    app.get("/leaderboard", async (req,res)=>{
      try{
        const leaderboard= await users.find({}).sort({ points: -1 }).limit(10).toArray();
